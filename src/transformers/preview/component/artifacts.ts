...
export const syncComponentArtifacts = async (handoff: Handoff): Promise<void> => {
  ...
  for (const componentId of runtimeIds) {
    ...
    validFiles.add('screenshot.png'); // generated by transformers/preview/component/screenshot.ts
    // Slot capability record from the build-time probe (transformers/plugins/slot-probe.ts). A real
    // build artifact, and it has to be listed here or the .json prune below deletes it — which it did,
    // silently, after the plugin had written it successfully. `-client.mjs` only survives because .mjs
    // is not a pruned extension.
    validFiles.add(`${componentId}-capabilities.json`);
