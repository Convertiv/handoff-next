...
import { COMPONENT_REFERENCE_SETTINGS, DESIGN_MD_SETTING_KEY, INCLUDE_FOUNDATIONS_SETTING_KEY } from './settings-constants';
      ...
      <div className="flex max-w-4xl flex-col gap-6 pb-10">
        ...
        <section className="rounded-xl border bg-background p-4 shadow-sm">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <label className="flex shrink-0 items-center gap-2 rounded-full border px-3 py-2 text-sm">
              <input
                type="checkbox"
          ...
          <div className="mt-4 rounded-lg bg-muted/40 p-3 text-sm text-muted-foreground">
            {foundationCount > 0 ? (
              <p>
                Current foundation context contains {foundations.colors.length} color(s), {foundations.typography.length} typography style(s),{' '}
                {foundations.effects.length} effect(s), and {foundations.spacing.length} spacing token(s).
              </p>
            ) : (
            ...
            <div className="mt-4">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {foundations.colors.slice(0, 24).map((color, index) => (
                  <span key={`${color.name}-${index}`} className="inline-flex items-center gap-1.5 rounded-md border px-2 py-1 text-xs" title={color.value}>
              ...
              return (
                <div key={setting.id} className="rounded-lg border bg-muted/20 p-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {image ? (
                      <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => removeComponentReference(setting)} aria-label={`Remove ${setting.label} reference`}>
