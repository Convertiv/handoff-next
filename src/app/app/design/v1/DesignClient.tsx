...
const DesignWorkbenchPage = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        router.replace(`${basePath}/design/v1`);
      } catch (e) {
  ...
  const handleUseLayoutGuide = useCallback(async () => {
    ...
    try {
      ...
      formData.append('image', layoutGuideImage);
      ...
      setLayoutGuideDescription(json.description.trim());
      setLayoutGuideWireframeUrl(json.wireframeImage?.trim() ?? '');
    } catch (e) {
  ...
  const getCanvasPoint = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    return {
      x: Math.min(Math.max(((event.clientX - rect.left) / rect.width) * CANVAS_WIDTH, 0), CANVAS_WIDTH),
      y: Math.min(Math.max(((event.clientY - rect.top) / rect.height) * CANVAS_HEIGHT, 0), CANVAS_HEIGHT),
    };
  };
  ...
  const createAnnotatedImageBlob = async (src: string, imageAnnotations: AnnotationRect[]) => {
    ...
    const imageWidth = sourceImage.naturalWidth || CANVAS_WIDTH;
    const imageHeight = sourceImage.naturalHeight || CANVAS_HEIGHT;
    const displayedScale = Math.min(CANVAS_WIDTH / imageWidth, CANVAS_HEIGHT / imageHeight);
    const displayedImageWidth = imageWidth * displayedScale;
    const displayedImageHeight = imageHeight * displayedScale;
    const displayedImageX = (CANVAS_WIDTH - displayedImageWidth) / 2;
    const displayedImageY = (CANVAS_HEIGHT - displayedImageHeight) / 2;
  ...
  const handleGenerate = async () => {
    ...
    if (
      ...
      setError(
        'Select an image on the canvas, attach a prompt image, add a Layout Guide, save component references in settings, use foundations, or add a custom foundation image in settings.'
      );
      return;
    }
    ...
    const submittedLayoutGuideImage = layoutGuideImage;
    const submittedLayoutGuideWireframeUrl = layoutGuideWireframeUrl;
    const submittedLayoutGuideDescription = layoutGuideDescription.trim();
                ...
                <Collapsible defaultOpen className="rounded-md border px-2 py-1">
                  ...
                  <CollapsibleContent className="space-y-2 pb-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Add a web section screenshot to use its layout as a wireframe.
                    </p>
                    ...
                    {layoutGuidePreviewUrl ? (
                      <>
                        ...
                        {layoutGuideWireframeUrl ? (
                          <div className="overflow-hidden rounded-md border bg-background">
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img src={layoutGuideWireframeUrl} alt="Generated layout wireframe" className="max-h-44 w-full object-contain" />
                          </div>
                        ) : null}
                      </>
                    ) : (
                  ...
                  {({ zoomIn, zoomOut, resetTransform }) => (
                    <>
                      ...
                      <TransformComponent wrapperClass="!h-full !w-full" contentClass="!h-fit !w-fit">
                        <div
                          className={`relative flex h-[1152px] w-[2048px] items-center justify-center ${isAnnotating ? 'cursor-crosshair' : ''}`}
                        ...
                        >
                          {imageSrc ? (
                            <Image
                              ...
                              unoptimized
                              className="h-full w-full max-w-none rounded-md bg-background object-contain shadow-lg"
                            />
                          ) : (
