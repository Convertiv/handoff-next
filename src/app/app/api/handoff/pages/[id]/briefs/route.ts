import { NextResponse, type NextRequest } from 'next/server';
import { auth } from '@/lib/auth';
import { computePermissions, toVisibility, type MutateActor } from '@/lib/authz/policy';
import { getResourceOwner } from '@/lib/db/grant-queries';
import { listBriefsForPage } from '@/lib/db/queries';

/**
 * The invitations (briefs) created from one page.
 *
 * Gated on `canView` of the **page**, because briefs have no independent visibility — they inherit their
 * parent's (see `docs/INVITE-TO-BUILD.md`). Asking about the page is therefore the whole authorization.
 */
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const session = await auth();
  if (!session?.user?.id) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });

  const { id } = await params;
  const pageId = (id ?? '').trim();
  if (!pageId) return NextResponse.json({ error: 'A page id is required.' }, { status: 400 });

  const owner = await getResourceOwner('pattern', pageId);
  if (!owner) return NextResponse.json({ error: 'Not found' }, { status: 404 });

  const actor: MutateActor = { userId: session.user.id, role: session.user.role ?? null };
  const perms = computePermissions(
    actor,
    { ownerUserId: owner.ownerUserId, visibility: toVisibility(owner.visibility) },
    null
  );
  if (!perms.canView) return NextResponse.json({ error: 'Not found' }, { status: 404 });

  try {
    return NextResponse.json({ briefs: await listBriefsForPage(pageId) });
  } catch (e) {
    console.error('[pages/:id/briefs]', e);
    return NextResponse.json({ error: 'Could not load invitations.' }, { status: 500 });
  }
}
