...
export default function IntegrationsSection() {
  ...
  const triggerFetch = async () => {
    ...
    try {
      ...
      if (!res.ok) { setStatusMessage(data.error ?? 'Failed to queue Figma fetch.'); return; }
      if (!data.jobId) { setStatusMessage('No fetch job id returned.'); return; }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>Connect your Figma account to fetch design tokens from the GUI.</CardDescription>
              </div>
            </div>
            {loadingStatus ? (
          ...
          {!oauthConfigured && !loadingStatus ? (
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-wrap items-center gap-3">
                ...
                >
                  {triggerBusy || jobActive ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <CheckCircle className="h-3.5 w-3.5" />}
              ...
              {connected && !loadingStatus ? (
                <p className="text-sm text-muted-foreground">
                  Your Figma account is linked. Click <strong>Run Figma Fetch</strong> to pull the latest design tokens.
                </p>
              ) : null}
            </div>
          )}
        ...
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>
                Server-side OpenAI key for the Playground &quot;Generate with AI&quot; wizard (optional BYOK still works when this is off).
              </CardDescription>
            </div>
          </div>
        </CardHeader>
