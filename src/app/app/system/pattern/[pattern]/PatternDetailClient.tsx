...
const formatPreviewLabel = (component: PatternListObject['components'][number], index: number) => {
  ...
  if (!previewKey) return { title: 'Default preview', subtitle: 'Uses the component default preview configuration.', rawKey: null };
  if (isSyntheticPatternPreview(previewKey)) return { title: component.args ? 'Custom configuration' : `Generated preview ${index + 1}`, subtitle: component.args ? 'Built from inline args for this pattern.' : 'Generated specifically for this pattern composition.', rawKey: previewKey };
  return { title: startCase(previewKey), subtitle: 'Named preview from the component docs.', rawKey: previewKey };
};
...
export default function PatternDetailClient({ id, menu, config, current, metadata, previousPattern, nextPattern }) {
  ...
  useEffect(() => {
    ...
    fetch(`${normalizedBasePath}/api/pattern/${id}.json`, { signal: controller.signal })
      ...
      .catch((err) => {
        if (err.name !== 'AbortError') setFetchError(err instanceof Error ? err.message : 'Failed to load pattern data.');
      });
    ...
    <Layout config={config} menu={menu} current={current} metadata={metadata}>
      <div className="flex flex-col gap-3 pb-14">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Badge variant="outline" className="rounded-full border border-gray-200 px-3 py-1 text-[11px] font-medium text-gray-600 dark:border-gray-700 dark:text-gray-300">
        ...
        {metadata.description && (
          <div className="prose max-w-[800px] text-xl font-light leading-relaxed text-gray-600 dark:text-gray-300">
            <ReactMarkdown components={MarkdownComponents} rehypePlugins={[rehypeRaw]}>{metadata.description}</ReactMarkdown>
          </div>
        )}
      </div>
          ...
          <div className="mt-8">
            ...
            <div className="mt-5 overflow-hidden rounded-xl border border-gray-200 dark:border-gray-800">
              {pattern.components.map((comp, i) => {
                ...
                return (
                  <div key={`${comp.id}-${i}`} className="border-b border-gray-200 bg-white last:border-b-0 dark:border-gray-800 dark:bg-transparent">
                    <div className="grid gap-4 px-5 py-4 md:grid-cols-[minmax(0,220px)_minmax(0,1fr)] md:px-6">
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        {comp.resolved === false && <Badge variant="warning" className="rounded-full px-2.5 py-0.5 text-[11px] font-medium">Unresolved</Badge>}
                      </div>
                      <div className="min-w-0">
                        {comp.resolved === false ? (
                          <div className="rounded-md bg-amber-50 px-3 py-2 text-sm text-amber-900 dark:bg-amber-950/40 dark:text-amber-200">Could not resolve component preview.</div>
                        ) : (
                          <>
                            <div className="space-y-2">
                              <div>
                                <p className="text-[10px] font-medium uppercase tracking-[0.1em] text-gray-400 dark:text-gray-500">Preview</p>
                              ...
                              {previewMeta.rawKey && !hasArgs && (
                                <div className="inline-flex max-w-full items-center gap-2 rounded-md bg-gray-50 px-2.5 py-1.5 dark:bg-gray-900">
                                  <span className="text-[10px] font-medium uppercase tracking-[0.12em] text-gray-400 dark:text-gray-500">Key</span>
                                      ...
                                      {argSummary.length > 0 && (
                                        <dl className="flex flex-wrap gap-2">
                                          {argSummary.map(([key, value]) => (
                                            <div key={key} className="inline-flex max-w-full items-center gap-2 rounded-md bg-gray-50 px-2.5 py-1.5 dark:bg-gray-900">
                                              <dt className="text-[10px] font-medium uppercase tracking-[0.12em] text-gray-400 dark:text-gray-500">{key}</dt>
                                          ...
                                          {Object.keys(comp.args ?? {}).length > argSummary.length && (
                                            <div className="inline-flex max-w-full items-center rounded-md bg-gray-50 px-2.5 py-1.5 text-xs text-gray-500 dark:bg-gray-900 dark:text-gray-400">+{Object.keys(comp.args ?? {}).length - argSummary.length} more</div>
                                          )}
                                        </dl>
                                      )}
                                      <AccordionTrigger className="inline-flex w-auto flex-none items-center gap-1 rounded-md bg-gray-50 px-2.5 py-1.5 text-[10px] font-medium uppercase tracking-[0.12em] text-gray-400 no-underline hover:bg-gray-100 hover:text-gray-600 hover:no-underline dark:bg-gray-900 dark:text-gray-500 dark:hover:bg-gray-800 dark:hover:text-gray-300 [&>svg]:h-3.5 [&>svg]:w-3.5">All args</AccordionTrigger>
                                    </div>
                                    <AccordionContent className="pt-2">
                                      <JsonTreeView data={comp.args} defaultExpanded maxInitialDepth={1} className="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-800 dark:bg-gray-900" />
                                    </AccordionContent>
                                  </AccordionItem>
                                </Accordion>
                              </div>
